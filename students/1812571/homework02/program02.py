...
def createTasksList(tasksDict, taskID):
    tasksList = list()
    previousTask = tasksDict[taskID]
    while (previousTask != "-1"):
        tasksList.append(previousTask)
        previousTask = tasksDict[previousTask]
    tasksList.reverse()
    
    return tasksList


def populateTasksDict(lines):
    currentTaskID = "-1"
    tasksDict = {}
    for line in lines:
        if (line[0:4] == "comp"):
            line = line[4:]
            currentTaskID = line
            tasksDict[currentTaskID] = "-1"
        elif (line[0:3] == "sub"):
            line = line[3:]
            tasksDict[currentTaskID] = line
            
    return tasksDict


def fillResultsDict(tasksDict, tasksIDToSearch):
    resultsDict = {}
    for el in tasksIDToSearch:
        if (el not in tasksDict):
            continue
        resultsDict[el] = createTasksList(tasksDict, el)
    return resultsDict        
...
def pianifica(fcompiti,insi,fout):
    ...
    tasksDict = populateTasksDict(fileContent.splitlines())
      
    resultsDict = fillResultsDict(tasksDict, insi)
             
    with open(fout,"w") as fo:
        json.dump(resultsDict,fo)