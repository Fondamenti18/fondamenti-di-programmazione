...
def pianifica(fcompiti,insi,fout):
    ...
    for x in insi:
        ...
        value=x
        while value!=' ':
            if value in DictCompSub.keys():
                ExistKey=True
                value=DictCompSub[value]
                if value!=' ':
                    a.append(value)
            else:
                value=' '
        ...
        if ExistKey:
            a.reverse()
            risultato[x]=a
    with open(fout, "w") as fjson:
        json.dump(risultato , fjson)
    fjson.close()
