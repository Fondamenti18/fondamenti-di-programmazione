...
def diz(fcompiti):    
    ...
    while pos<len(l):
        if 'comp' in l[pos]:
            l[pos]=l[pos].replace('c','')
            l[pos]=l[pos].replace('o','')
            l[pos]=l[pos].replace('m','')
            l[pos]=l[pos].replace('p','')
            l[pos]=l[pos].replace(' ','')
            l[pos]=l[pos].replace('\n','')
            d.update({l[pos]:'0'})
        else:
            l[pos]=l[pos].replace('s','')
            l[pos]=l[pos].replace('u','')
            l[pos]=l[pos].replace('b','')
            l[pos]=l[pos].replace(' ','')
            l[pos]=l[pos].replace('\n','')
            d[l[pos-1]]=l[pos]
        pos+=1
...
def ordine(d,insi):
    ...
    for x in insi:
        if x in d.keys():
            risultato.update({x:[d[x]]})
            if risultato[x][len(risultato[x])-1] in d.keys():
                while d[risultato[x][len(risultato[x])-1]]!='0':
                    
                    risultato[x].append(d[risultato[x][len(risultato[x])-1]])
    
    
    for x in risultato.keys():
        if len(risultato[x])>1:
            risultato[x]=risultato[x][::-1]
        if risultato[x]==['0']:
            risultato[x]=[]
