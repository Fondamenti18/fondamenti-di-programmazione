...
def dicted(l_righe,insieme): #funzione che crea un dizionario
    dizionario={}
    l_righe+='fine'
    i = 0
    while i < len(l_righe):
        if 'comp' in l_righe[i] and not 'sub' in l_righe[i + 1]: #come chiave usa il numero che è dopo 'comp'
            var = l_righe[i].split()
            dizionario[var[1]] = []
            i += 1
        elif 'comp' in l_righe[i] and 'sub' in l_righe[i + 1]:#come valore assegna, se c'e', il numero dopo 'sub'
            var1 = l_righe[i].split()
            var2 = l_righe[i + 1].split()
            dizionario[var1[1]] = var2[1]
            i += 2
        else:
            break
    return elaborate(dizionario,insieme)

def elaborate(dizionario,insieme): #funzione che torna un dizionario
    chiavi=dizionario.keys()
    risultato={}
    for i in chiavi:
        if i in insieme: #se la chiave i e' nell insieme delle chiavi da cercare
            key=i
            risultato[key]=[]
            if dizionario[key]!=[]: #se il valore assegnato alla chiave non e' vuoto
                l_value = []
                value=dizionario[key]
                l_value.insert(0,value)
                while dizionario[value]!=[]: #finche' il valore non è vuoto cerca la chiave nel dizionario
                    l_value.insert(0,dizionario[value]) #inserisce nella lista dei valori il valore assegnato alla chiave
                    value=dizionario[value] #sostituisce la chiave da cercare con il valore assegnato alla chiave precedente
                risultato[key]=l_value #assegna alla chiave iniziale tutti i valori trovati fin ora
    return risultato



def pianifica(fcompiti,insi,fout):
    with open(fcompiti,encoding='utf-8') as doc: #apre il file
        doc=doc.read().replace('comp',' comp ').replace('sub',' sub ').splitlines() #crea una lista delle righe del testo separando 'comp' e 'sub' con spazi
        risultato=dicted(doc,insi) #crea il dizionario completo e lo salva in risultato
    with open(fout,'w',encoding='utf-8') as f: #salva il risultato in fout
