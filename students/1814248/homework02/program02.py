...
def pianifica(fcompiti,insi,fout):
	...
	for i in range(0,len(testo)):
		testo[i]=testo[i].replace(' ', '').replace('\n','')
		if 'comp' in testo[i]:
			if i!=len(testo)-1:
				if 'sub' in testo[i+1]:
					diz[testo[i].replace('comp','')]=testo[i+1].replace('sub','').replace(' ', '').replace('\n','')
					i+=1
				else:
					diz[testo[i].replace('comp','')]=[]
			else:
					diz[testo[i].replace('comp','')]=[]
	for elem in insi:
		if elem in diz:
			diz2[elem]=trovasub(elem,diz)
	risultato=diz2
	with open(fout,'w') as file_json:
		json.dump(risultato, file_json)
	
def trovasub(elemento,dizionario):
	lista=[]
	while True:
		if dizionario.get(elemento) != []:
			lista.insert(0,dizionario.get(elemento))
			elemento=dizionario.get(elemento)
		else:
			break
	return lista
