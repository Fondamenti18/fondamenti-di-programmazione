...
def pianifica(fcompiti,insi,fout):
    compiti = open(fcompiti, 'r')
    find = []
    result = {}
    compitiList = compiti.read().replace(' ', '').split()

    for id in insi:
        if ('comp'+id in compitiList):
            find.append(id)
    for i, c in enumerate(compitiList):
        for f in find:
            if(i +1 < len(compitiList)):


                if('comp' + f  in c and compitiList[i +1].find('sub') != -1 and c.endswith('comp' + f)):
                    sub = compitiList[i +1].replace('sub', '')
                    newArray = []
                    newArray.append(sub)
                    if('comp'+sub in compitiList and 'comp'+sub != compitiList[-1]):
                        indice = compitiList.index('comp'+sub);
                        if(compitiList[indice + 1].startswith('sub')):
                            inx = compitiList[indice + 1].replace('sub', '')
                            newArray.append(inx)

                    result[f] = newArray

                elif ('comp' + f in c.replace(' ', '') and compitiList[i +1].find('comp') != -1):
                    result[f] = []

    with open(fout, 'a') as the_file:
        the_file.write(str(result).replace("'", '"'))
