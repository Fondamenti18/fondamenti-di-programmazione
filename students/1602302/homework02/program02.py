...
def estraisubs(dic,comp):
    ...
    while dic[compito]!="":
        appoggio=dic[compito]
        compito=appoggio
        listaID.append(appoggio)
    listaID.reverse()
    
    return listaID
       

def stripword(parola):
    parola=parola.replace("comp","").replace("sub","").strip()
    return parola

  
def pianifica(fcompiti,insi,fout):
    
    fi=open(fcompiti,"r",-1,"utf-8")
    ...
    for line in fi:
        ...
        if "comp" in line:
            compID=stripword(line)
            dizionariocompiti[compID]=subID
        if "sub"  in line:
            subID=stripword(line)
            dizionariocompiti[compID]=subID
            subID=""
    fi.close()
    
    for compcheck in insi:
        if estraisubs(dizionariocompiti,compcheck) != "":    
            dizionariooutput[compcheck]=estraisubs(dizionariocompiti,compcheck)
    
    output=open(fout,"w")
    json.dump(dizionariooutput,output)
