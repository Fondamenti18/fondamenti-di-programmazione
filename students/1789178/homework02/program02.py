...
def scomponiSub(ls,d):
    v=[]
    for index,val in enumerate (ls):
        v=ls[index].split('sub')
        if( len(v)>1):
            d[v[0].replace('\n','')]=[v[1].replace('\n','')]
        else:
             d[v[0].replace('\n','')]=[]
...
def pianifica(fcompiti,insi,fout):
    ...
    for x in d:
        if(x in insi):
            ddd[x]=trovasub(d,x,x,dd)

    json.dump(ddd,j)
    j.close()
    f.close()
    #return ddd
#print(pianifica('file02_50000_100.txt','set([str(i) for i in range(1000,2001)])', 'a.json'))