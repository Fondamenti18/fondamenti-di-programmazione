...
Gli (ID) devono comparire nella lista nell'ordine di esecuzione corretto, dal primo fino a quello precedente 
a quello richiesto 
(ovviamente il primo ID di una lista non vuota corripondera' sempre ad un compito che non richiede un compito 
preliminare). 
...
def pianifica(fcompiti,insi,fout):
    ...
    for linenr in range(len(data)):
        if 'comp' in data[linenr]:
            for number in data[linenr].split():
                number = isthisnumber(number)
                if number!='-1' and number!='':
                 
                    dicto[number]=[]
                    if linenr+1<len(data):
                        if 'sub' in data[linenr+1]:
                          
                            for subnumber in data[linenr+1].split():
                                subnumber = isthisnumber(subnumber)
                                if subnumber!='1':
                                    dicto[number] = dicto[number] + subnumber.split()

    solutiondicto = {}
    for x in insi:
        if x in dicto.keys():
            solutiondicto[x] = dicto[x]
            ind=0
            try:
                while 1:
                    y = solutiondicto[x][ind]
                  
                    if y in dicto.keys():
                        solutiondicto[x] = solutiondicto[x] + dicto[y]
                    ind=ind+1
            except IndexError:
                solutiondicto[x] = solutiondicto[x][::-1]
                continue
    print("This is the SOLUTION dictionary of all comp in the file\n",solutiondicto)
    import json
    with open(fout, 'w') as fp:
        json.dump(solutiondicto, fp)
