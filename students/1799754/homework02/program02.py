...
def struttura(lista):
    ...
    for i in range(len(lista)):
        if lista[i].startswith("comp"):
            if(i+1<count and lista[i+1].startswith("sub")):
                d[lista[i][4:]]=lista[i+1][3:]
            else:
                d[lista[i][4:]]=" " 
    return d

def compilazione(num,d,l):
        while True:
            if d.get(num)==' ' or (num not in d) :
                return list(l)
            else:
                l.insert(0,d[num])
                num=d[num]  
...
def pianifica (fcompiti,insi,fout):
    lista = []
    f= open(os.path.join(os.path.dirname(sys.argv[0]), fcompiti))
