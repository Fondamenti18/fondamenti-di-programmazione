...
def decodificatore(conf):
    ...
    n=conf[0]
    if len(conf)==2:
        findnewcode(tuple(conf[-1][0]),conf[-1][1],n)
        return choice(temp)
    elif len(conf)>2:
        candidates=check(temp,conf[1:])
        temp=candidates
        return choice(temp)
    else:return sample([x for x in range(10)],n)

def findnewcode(ls,tup,n):
    global temp
    hold=tup[0]+tup[1]
    filt=[]
    for j in combinations(ls,hold):
         for i in combinations([x for x in range(10) if x not in ls],n-hold):
            filt+=list(permutations(j+i))
    temp=filt      
    return 
...
def check(ls,conf):
    ...
    for i in ls:
        ...
        for j in range(len(conf)):
            a,b=control(i,conf[j][0])
            if (a,b)==conf[j][1] or (b,a)==conf[j][1]:k+=1
        if k==len(conf):filt.append(i)   
    return filt

def control(code,trying):
    a=0
    b=0
    for i in range(len(code)):
        if code[i]==trying[i]:a+=1
        elif code[i] in trying:b+=1
    return a,b
