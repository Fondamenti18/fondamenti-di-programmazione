...
ATTENZIONE: Se un test del grader non termina entro 10 secondi il punteggio di quel test e'
'''
...
def testo(fcompiti):
    testo1=[]
    with open(fcompiti,encoding=('utf-8-sig')) as f:
       testo=f.readlines()
       for i in testo:
           g=i.split()
           g=''.join(g)
    ...
    if i in qls:
        if qls[-1]==i:
            ...
            if 'sub' in v:
                unl=unl+(v[3:])
    return unl
def count(fcompiti,i):
    a=idcomp(testo(fcompiti))
    dic={}
    lista=[]
    lista.append(i)
    while True:
        for x in lista:
            su=ocount(fcompiti,x)
            if x not in a:
                return 0
            if su == '':
            ...
            if len(su) != 0 :
                lista.append(su)
    return lista         
def capz(fcompiti,insieme):
    dic={}
    for i in insieme:
        if count(fcompiti,i)==0:
            continue
        dic.update({i:count(fcompiti,i)})
    return dic
def pianifica(fcompiti,insi,fout):
    '''Implementare qui la funzione'''
    dic=capz(fcompiti,insi)
    with open(fout,'w',encoding='utf-8') as fgiulio:
        json.dump(dic,fgiulio)
