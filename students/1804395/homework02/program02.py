...
def find(element, position, eccolo):
    try:
        eccolo.append(str(position[int(element)]))
        return find(str(position[int(element)]),position,eccolo)
    except KeyError:
        pass
...
def pianifica(fcompiti,insi,fout):
    ...
    with open(fcompiti) as f:
        lines=(f.read()).split('comp')

    for i in lines:
        listOfLines.append([int(x.group()) for x in re.finditer(r'\d+',i)])
    ...
    for i in listOfLines:
        ...
        if len(i) == 1:
            workSet.add(i[0])
    dictionary=dict(listOfComp)

    for i in insi:
        if int(i) in workSet:
            lista=[]
            find(int(i),dictionary, lista)
