...
def ai(griglia_corrente, griglia_precedente, x, y, vx, vy, car, verso, startx, starty):
    ...
    if vx==vy and vx==+2:
        ...
        if (x==startx and y==starty) or griglia_precedente[y][x]=="|":
            if griglia_precedente[y][x]=="|":
                vel=(vx*-1,vy*-1)
            else:
                vel=(verso,0)
        else:
            new_vel=trova_percorso(x,y,vx,vy,griglia_corrente)
            if (vx,vy)==new_vel:
                if griglia_corrente[y+vy][x+vx]!="0":
                    vel=(0,0)
                else:
                    vel=(vx,vy)
            else:
...
def trova_percorso(x,y,vx,vy,griglia_corrente):
    d=dict()
    if vx==-1 and vy==-1:
        lst=[(-1,0),(-1,-1),(0,-1)]
    elif vx==-1 and vy==0:
    ...
    for i in lst:
        ...
        b=y
        cont=0
        while griglia_corrente[b][a]!="#":
            if cont==1 and griglia_corrente[b][a]=="O":
                del d[i]
                break
            b=b+i[1]
            a=a+i[0]
