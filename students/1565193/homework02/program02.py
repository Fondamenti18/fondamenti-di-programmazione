...
def pianifica(fcompiti,insi,fout):
    ...
    with open (fcompiti,"r",encoding='utf-8') as compiti: risultparz=primodiz(compiti,risultparz)
        
    risultato=aggrega(insi,risultparz,risultato)
    risultato=str(risultato).replace("'",'"')

    with open (fout,"w",encoding='utf-8') as ris: ris.write(risultato)

def primodiz(compiti,risultparz):
...
def aggrega(insi,risultparz,risultato):
    
    complist=[]
    
    for i in insi:           
        
        if i in risultparz.keys():
            k=i
            
            while risultparz[i]!="":
                complist.append(risultparz[i])
                i=risultparz[i]
            
            complist.reverse()
            risultato.setdefault(k,complist)
            complist=[]
