...
def dizi(lis):
    ...
    for i in range(len(lis)):
        if lis[i].__contains__('comp'):
            k=extract(lis[i])
            if i+1<len(lis) and lis[i+1].__contains__('sub'):
                d[k]=[extract(lis[i+1])]
            else:
                d[k]=[]
    return d
...
def percorso(d,k,l):
    if len(d[k])!=0:
        l.insert(0,d[k][0])
        return percorso(d,d[k][0],l)
    else:
        return l
        
def perc(d,k):
    return percorso(d,k,list())
                   
def pianifica(fcompiti,insi,fout):
    d=dizi(op(fcompiti))
    d1={}
    for e in insi:
        if d.keys().__contains__(e):
            l=perc(d,e)
            d1[e]=l
    documento=open(fout,'w')
    j_s=json.dumps(d1)
    documento.write(j_s)
    documento.close()
