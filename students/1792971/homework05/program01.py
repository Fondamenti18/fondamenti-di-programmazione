...
def decodificatore(configurazione):
    ...
    if l<11:
        if l==1:
            return [0]*configurazione[0]
        elif l==2:
            return [1]*configurazione[0]
        elif l==3:
            return [2]*configurazione[0]
        elif l==4:
            return [3]*configurazione[0]
        elif l==5:
            return [4]*configurazione[0]
        elif l==6:
            return [5]*configurazione[0]
        elif l==7:
            return [6]*configurazione[0]
        elif l==8:
            return [7]*configurazione[0]
        elif l==9:
            return [8]*configurazione[0]
        elif l==10:
            return [9]*configurazione[0]
    else:
        seq=""
        for i in range(1,11):
            if configurazione[i][1]!=(0,0):
                seq+=str(configurazione[i][0][0])
        print(seq)
                
        presente=True
        while presente:
            risp=[]
            for i in range(configurazione[0]):
                c=random.choice(seq)
                risp+=[int(c)]
                seq=seq.replace(c,"")
            for i in range(1,l):
                if risp in configurazione[i]:
                    presente=True
                else:
                    presente=False
        return risp           