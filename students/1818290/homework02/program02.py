...
def pianifica(fcompiti,insi,fout):
    controllocompiti(fcompiti,insi)     
    dizsemp=rimuovi(diz,insi)
    risultato=sequenze(dizsemp,diz)
...
def controllocompiti(fcompiti,file):
    file=open(fcompiti)
    compnum=0
    for line in file:        
        taglio=line.replace('p',' ').replace('b',' ').split() 
        if taglio[0]=='com':
            diz[taglio[1]]=[]
            compnum=taglio[1]
        if taglio[0]=='su':
            diz[compnum].append(''.join(taglio[1]))         
    file.close()
...
def sequenze(dizsemp,diz):

    for key in dizsemp:

        if dizsemp[key]!=[]:
            for x in dizsemp[key]:                
                try:                    
                    dizsemp[key].append(''.join(diz[x]))
                except:
                    pass
        dizsemp[key]=dizsemp[key][::-1]
    
    for a in dizsemp.values():
        try:
            a.remove('')
        except ValueError:
            pass 
