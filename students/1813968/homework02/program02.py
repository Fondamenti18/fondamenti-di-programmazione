...
def pianifica(fcompiti,insi,fout):
	dizionario=crea_dizionario(fcompiti)
	insieme=list(insi)
	dizionario_2={}
	for i in insieme:
		l=[]
			...
			if dizionario[i]==[]:
				...
				for c in l:
					if c in dizionario:
						if dizionario[c]!=[]:
							l+=dizionario[c]
				dizionario_2[i]=list(reversed(l))
	with open(fout,"w") as File:
		json.dump(dizionario_2,File)
			
			
			
def crea_dizionario(fcompiti):
		...
		for i in Testo:
			if len(re.findall(r"\d+",i))==1:
				dizionario.setdefault((re.findall(r"\d+",i))[0],[])
			elif len(re.findall(r"\d+",i))==2:
				dizionario[(re.findall(r"\d+",i))[0]]=[re.findall(r"\d+",i)[1]]
		return dizionario
...
"""def c(l,dizionario):
	...
	while (j+1)==len(l):
		c=l[j:]
		if str(c[0]) in dizionario:
			if dizionario[str(c[0])]!=[]:
				l+=dizionario[str(c[0])]
		j+=1
	return l"""
