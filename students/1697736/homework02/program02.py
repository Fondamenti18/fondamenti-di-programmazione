...
def pianifica(fcompiti,insi,fout):
    ...
    with open(fcompiti, encoding='utf-8-sig') as f:
        ...
        for i,n in enumerate(fr):
            if 'comp' in fr[i] and 'sub' in fr[i+1]:
                doptupla = (soloNumero(fr[i+1]),soloNumero(fr[i]))
                insieme.add(doptupla)
                nume.add(soloNumero(fr[i+1]))
                nume.add(soloNumero(fr[i]))
            elif 'comp' in fr[i]:
                tupla = (soloNumero(fr[i]),)
                insieme.add(tupla)
                nume.add(soloNumero(fr[i]))
        for n in insi:
            lista = []
            ...
            if n in nume:
                while not trovato:
                    for m in insieme:
                        if len(m) == 1 and m[0] == ultimo:
                            trovato = True
                            break
                        elif len(m) > 1 and m[1] == ultimo:
                            ultimo = m[0]
                            lista.append(m[0])
                lista.reverse()
                diz[n] = lista
