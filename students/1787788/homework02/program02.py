...
'''
def indicedizionario(content):
        dizionario = dict()
        numerocomp = '0'
        numerosub = '0'
        for line in content:
                if 'comp' in line:
                        numerocomp =  re.search(r'\d+', line).group()
                        dizionario[numerocomp] = []
                else:
                        numerosub = re.search(r'\d+', line).group()
                        dizionario[numerocomp] = numerosub
        return dizionario

def giradizionario(valore, dizionario):
        indice = valore
        sub = list()
        while(dizionario[indice] != []):
                sub.insert(0,dizionario[indice])
                indice = dizionario[indice]
        return sub
...
def pianifica(fcompiti,insi,fout):
	...
	with open(fcompiti, 'r') as f:
		content = f.readlines()
		newdizionario = indicedizionario(content)
	for i in insi:
                if i in newdizionario:
                        lista = giradizionario(i, newdizionario)
                        dizionariofinale[i] = lista
	with open(fout, 'w') as fp:
		json.dump(dizionariofinale, fp)
