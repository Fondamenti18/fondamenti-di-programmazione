...
def classifica(testo):
    lista = [re.sub('\s+', "", riga) for riga in testo.split("\n")]
    pairs = {}
    i = 0
    while i < len(lista)-1:
        if lista[i].startswith('c') and lista[i+1].startswith('s'):
            pairs[estrai_num(lista[i])] = estrai_num(lista[i+1])
            i += 2
        else:
            pairs[estrai_num(lista[i])] = ""
            i += 1
    return pairs

def lista_sub(mappa, k):
    sub = []
    while True:
        if mappa[k] == "":
            sub = [k] + sub
...
def estrai_num(riga):
    return re.findall(r'\d+', riga)[0]

def pianifica(fcompiti, insi, fout):
    with open(fcompiti, 'r') as f:
        testo = f.read()
    ret = {}
    compiti = classifica(testo)
    for comp in insi:
        if comp in compiti:
            ret[comp] = lista_sub(compiti, comp)
