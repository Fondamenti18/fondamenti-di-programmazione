...
def decodificatore(configurazione):
    ...
    #CONTROLLO NUMERI PRESENTI SICURAMENTE NEL CODICE
    
    if len(elementi) == 0:
        for i in x:
            elementi.append(i)
    
    if 'FINE' in controllo and len(elementi) == 1:
        valori_trovati.append(elementi[0])
        del elementi[0]
        
       
        
        
    if len(valori_trovati) != n and not 'FINE' in controllo: # Si esegue solamente se il controllo non e finito
        ...
        #GENERA UN CODICE CASUALE CO I NUM
        if len(controllo2) == 0:
            global permutazioni

            if len(permutazioni) == 0:
                #permutazioni.append(list(itertools.permutations(valori_trovati)))
                permutazioni = list(itertools.permutations(valori_trovati,n))
                risposta = random.choice(permutazioni)
                primo.append(risposta)
                controllo2.append('d')
            ...
            if len(controllo2) > 0:
                ...
                for el in range(len(permutazioni)-1,-1,-1): 
                    #print(type(permutazioni[el]))
                    lastTried = configurazione[-1][0]
                    resp = risposta2(lastTried, list(permutazioni[el]))
                    c = resp[0]
                    d = resp[1]
                    a = tupla[0]
                    b = tupla[1]
                    
                    if  not (a == c and b == d) or( a == d and b == c ):
                ...
                if len(permutazioni) == 1:
                    print(permutazioni)
                    controllo.clear()
                    valori_trovati.clear()
                ...
                return list(permutazioni[0])
        
        

def risposta2(codice, proposta):
