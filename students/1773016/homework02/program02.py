...
def dizCompiti(file):
    ...
    for riga in fin:
        if 'comp' in riga:
            ...
            precedente = getID(riga)
            diz[precedente] = ''
        else:
            riga = riga.replace('sub','')
            diz[precedente] = getID(riga)
    fin.close()
    return diz
def funRicorsiva(dizFile,listaOut,temp):
    if dizFile[temp]!='':
        listaOut.append(dizFile[temp])
        funRicorsiva(dizFile,listaOut,dizFile[temp])
def initDiz(insieme,dizFile,dizOut):
    for elem in insieme:
        if elem in dizFile.keys():
            dizOut[elem] = []
            funRicorsiva(dizFile,dizOut[elem],elem)
            dizOut[elem].reverse()
def pianifica(fcompiti,insi,fout):
    fileWrite = open(fout,'w',encoding='UTF-8')
    dizionarioCompiti = dizCompiti(fcompiti)
    dizOut = {}
    initDiz(insi,dizionarioCompiti, dizOut)
    print(json.dumps(dizOut),file=fileWrite)
    fileWrite.close()
