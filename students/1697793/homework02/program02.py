...
def pianifica(fcompiti,insi,fout):
    ...
    inFile = open(fcompiti, 'r', encoding='utf8')
    lines = inFile.readlines()
    inFile.close()
    
    finalJson = {}
    homeworks = {}
    currentHomeworkId = None
    
    for line in lines:
        currentLine = line.strip()
        if 'comp' in currentLine:
            homeworkId = currentLine.split('comp')[1].split()[0]
            homeworks[homeworkId] = ''
            currentHomeworkId = homeworkId
        elif 'sub' in currentLine:
            homeworkId = currentLine.split('sub')[1].split()[0]
            homeworks[currentHomeworkId] = homeworkId 
    
    for homeworkId in insi:
        path = getPath(homeworkId, homeworks, [])
        if path != None:
            finalJson[homeworkId] = path[::-1]
