...
def pianifica(fcompiti,insi,fout):
    ...
    try:
        print ('Apro il file', fcompiti, 'di input\n')
        file_input = codecs.open(fcompiti, "r", "utf-8" )

    except:
        print ('Il file', fcompiti, 'non è stato trovato')
        print(' ')
    
    print('insi:', insi)
    print(' ')
   
    with open('risTest1.json', 'r') as f:
        datax = json.load(f)

    print('Risultato atteso:',datax, '\n')
    ...
    for line in file_input:
        ...
        if tipo == 'comp': 
            ...
            if chiave in insi:
                trovato = True
                print(' ')
                print (chiave, 'sta in insi')
                print(' ')
                dizionario_risultato [chiave] = []
                ...
                trovato = False
                print(' ')
                print('La chiave non ci interessa')
                print(' ')
                
        elif tipo == 'sub' and trovato:
            print(' ')
            print (chiavesub, 'è propedeutico al precedente')
            print(' ')
            listasub += chiavesub
            print (listasub)
            print(' ')
            ...
            for line in file_input:
                ...
                if line.startswith('comp') and chiave in listasub:
                    print ('daje')
                    dizionario_risultato[n] = (chiave)
                    print (dizionario_risultato)
                    trovato = False
                    listasub.remove(chiave)
                    break
