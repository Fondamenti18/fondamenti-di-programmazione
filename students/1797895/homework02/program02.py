...
def aperturaFILE(file):
    f=open(file)
    return f 

def createD(nomefile):
    insieme = {}
    stringa=''
    righe = [""]
    ...
    for riga in nomefile:
        righe.append(riga)
        if "sub" in riga:
            insieme[conv(stringa,righe[-2])] = conv(stringa,riga)
        if "comp" in riga:
            insieme[conv(stringa,riga)] = []
    return insieme
 


def conv(stringa,listasub):
    for a in listasub:
        if a.isdigit() == 1:
            stringa=stringa+a
    return stringa
...
def funBase(lista, x, d):
    if x == []:
        return lista[1:]
    lista.insert(0, d[x])
    return funBase(lista, d[x], d)


def pianifica(fcompiti, insieme, fout):
    f=aperturaFILE(fcompiti)
    diz=createD(f)
    ...
    for comp in insieme:
        if comp in diz:
            insiemeF[comp] = funBase([], comp, diz)
    json.dump(insiemeF,open(fout,'w'))
