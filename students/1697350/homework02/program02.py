...
def pianifica(fcompiti,insi,fout):
    ...
    # reading data and assigning exams to sub exams
    with open(fcompiti) as f:
        value = None
        for line in f:
            line = line.strip()
            if "comp" in line:
                value = line.split("comp")[1].strip()
                dizionario[value] = []
            if "sub" in line:
                value_sub = line.split("sub")[1].strip()
                dizionario[value].append(value_sub)
    # appending path until we reach the end (a key of the dict with empty list)
    ...
    for element in insi:
        if element in dizionario:
            ...
            while not end:
                value = dizionario[element_old]
                if len(value) == 0:
                    end = True
                else:
                    finalDict[element].append(value[0])
                    element_old = value[0]
    # reverting the paths
    dizionario3 = defaultdict(list)
    for key, value in finalDict.items():
        newList = []
        for i in list((reversed(value))):
            if i not in newList:
                newList.append(i)
        dizionario3[key] = newList
    with open(fout,'w') as out:
        json.dump(dizionario3, out)
