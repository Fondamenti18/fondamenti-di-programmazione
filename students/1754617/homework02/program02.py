...
def pianifica(fcompiti,insi,fout):
    ...
    with open(fcompiti) as f:
        ...
        listaTesto=testo2.splitlines()
        diz=creaDizionario(listaTesto)
        for chiave in insi:
            lista=[]
            try:
                if(diz['comp'+chiave]=='TURBO'):   #parola chiave che mi riconosce
                    dizionario[chiave]=lista
                    continue
                else:
                    chiave2=chiave
                    while((diz[('comp'+chiave2)])[0]=='s'):
                        lista.append(diz[('comp'+chiave2)][3:])
                        chiave2=(diz[('comp'+chiave2)])[3:]
                    lista.reverse()
                    dizionario[chiave]=lista
            except KeyError:
                continue
    ...
    with open(fout,'w') as v:
        json.dump(dizionario,v)


def creaDizionario(lista):  #creo dict per velocizzare la ricerca
    lungh=len(lista)
    dizionario2={}
    x=0
    try:
        while(x<lungh):
            if(lista[x+1][0]=='s'):
                dizionario2[lista[x]]=lista[x+1]
                x=x+2
            else:
                dizionario2[lista[x]]='TURBO'   #parola chiave
                x=x+1
        return dizionario2
    except IndexError:
        dizionario2[lista[x]]='TURBO'
        return dizionario2
