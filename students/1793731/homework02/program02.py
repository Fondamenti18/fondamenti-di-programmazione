...
def pianifica(fcompiti,insi,fout):
    compiti=dict()
    compitiToJson=dict()     
    compitiPreliminari=list()    
    compitoIdPrecedente=None
    compitoPreliminare="0"
    f=open(fcompiti,'r')
    for stringa in f.readlines():
        indice=stringa.find("comp")
        if(indice>=0):
            compitoId=stringa[indice+4:].strip()
            if compitoIdPrecedente!=None:
                compiti[compitoIdPrecedente]=compitoPreliminare
                compitoPreliminare="0"              
            compitoIdPrecedente=compitoId
        else:
            indice2=stringa.find("sub")
            compitoPreliminare=stringa[indice2+3:].strip()
    ...
    for compito, preliminare in compiti.items():

        if compito in insi:        
            compitiPreliminari=list()
            if preliminare!="0":
                compitiPreliminari.append(preliminare)
                while True:
                    uscita=True
                    if preliminare in compiti.keys():
                        preliminare=compiti.get(preliminare)
                        if preliminare !='0':
                            compitiPreliminari.append(preliminare)
                        uscita=False
                    if uscita==True:
                        break
                compitiPreliminari.reverse()
                compitiToJson[compito]=compitiPreliminari; 
            else:
                compitiToJson.update({compito:[]})

    fileOut=open(fout,'w', encoding='utf8')
    json.dump(compitiToJson, fileOut)
    fileOut.close()
