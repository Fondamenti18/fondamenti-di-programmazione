...
def apri(fcompiti):
    file = open(fcompiti, encoding='utf-8')
    testo = file.read()
...
def nuo(fcompiti):
    ...
    for i in range(len(a)):
        if 'comp' in a[i] and'sub' in a[i + 1]:
            nuova.update({a[i] : a[i + 1]})
        elif 'comp' in a[i] and'sub' not in a[i + 1]:
            nuova.update({a[i] : 'x'})
    return nuova
#print(nuo(fcompiti))            
            
def num(fcompiti):
    #k = dict(zip(nuo(fcompiti)[::2],nuo(fcompiti)[1::2]))
    k = {}
    k.update({c.replace('comp','') : v.replace('sub', '') for c, v in nuo(fcompiti).items()   })
    k = {c.replace(' ','') : v.replace(' ', '') for c,v in k.items()}
    return k            
#print(num(fcompiti))

def spero(fcompiti, insi):
    abc = {k : [v] for k,v in num(fcompiti).items() if k in insi}
    return abc
#print(spero(fcompiti,insi))
def s(f,insi,fcompiti):
    
    a = spero(fcompiti, insi)
    for k,v in a.items(): 
        c =  v
        for m in c:
            if str(m) in f:
    ...
    for k,c in a.items():
       ...
       c.reverse()
    return a
#print(s(fcompiti,insi))             

import json
def pianifica(fcompiti, insi, fout):
    with open(fout, 'w') as l:
        json.dump(s(num(fcompiti),insi,fcompiti),l)
