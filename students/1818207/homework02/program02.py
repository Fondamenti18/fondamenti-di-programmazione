...
def pianifica(fcompiti,insi,fout):
    ...
    risultato = {}
    lista = []
    estrazione = open(fcompiti,"r",-1,"utf-8").read()
    mappatura = {}
    numest = re.findall(r'\d{1,}',estrazione)
    texest = re.findall(r'[a-z]+',estrazione)
    x = 0
    for ogniid in numest :
        x += 1
    for i in range(len(texest)):
        ...
        if i < len(texest)-1 :
            if texest[i+1] != 'comp' :
                mappatura[numest[i]] = numest[i+1]
  
    for key in cerca :
        key1 = key
        if key1 in mappatura:
            while mappatura[key1] != '' :
                insiord = mappatura[key1]
                lista.append(insiord)
                key1 = insiord
            else :
                lista.reverse()
                risultato[key] = lista
