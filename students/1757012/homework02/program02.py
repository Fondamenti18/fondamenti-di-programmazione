...
def dalistaadiz(dizionarioebasta,listanormalizzata):
    for i in range(0,len(listanormalizzata)-1):
        a=listanormalizzata[i]
        b=listanormalizzata[i+1]
        if a[:4]=="comp" and b[:3]=="sub":
            dizionarioebasta[a[4:]]=b[3:]
...
def pianifica(fcompiti,insi,fout):
    filecompiti=open(fcompiti,"r")
    listanormalizzata=[]
    dizionarioout={}
    dizionarioebasta={}
    with filecompiti:
        righefile=filecompiti.readlines()
        for i in range(0,len(righefile)):
            listanormalizzata.append(righefile[i].replace("\n","").replace(" ",""))
    dalistaadiz(dizionarioebasta,listanormalizzata)
    for i in range(0,len(insi)):
        insieme=insi.pop()
        b=insieme
        libero=False
        if b not in dizionarioebasta:
            if "comp"+b in listanormalizzata:
                libero=True
            else:
                continue
        listasottocompiti=[]
        j=True
        while j:
            if b in dizionarioebasta:
                listasottocompiti.append(dizionarioebasta[b])
                c=dizionarioebasta[b]
                b=c
            else:
                j=False
        if libero and listasottocompiti==[]:
            dizionarioout[insieme]=[]
        elif listasottocompiti!=[]:
            listasottocompiti.reverse()
            dizionarioout[insieme]=listasottocompiti
            
    with open(fout,"w") as fileout:
        json.dump(dizionarioout,fileout)
