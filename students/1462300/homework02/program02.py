...
def pianifica(fcompiti,insi,fout):
    ...
    stringa_numero = ''
    stringa_sub = ''
    with open(fcompiti, 'r', encoding='utf-8') as p:
        righe = p.readlines()
    for ele in range(len(righe)):
        righe[ele] = righe[ele].strip('\n')    
    stringa_compiti = ''.join(righe)
    for lettera in stringa_compiti:
        if lettera != ' ':
            nuovastringa += lettera
    listae = nuovastringa.split('comp')
    for u in range(len(listae)-1):
        if listae[u] == '':
            del(listae[u])
        for ele in listae:
            if 'sub' in ele:
                ...
                stringa_sub = ''
                while ele[i].isdigit():
                    stringa_numero += ele[i]
                    i +=1
                i += 3
                stringa_sub += ele[i:]
                lista_sub = list(stringa_sub)
                compreli[stringa_numero] = lista_sub
            else:
                compreli[ele] = []
    print(compreli)
    for d in compreli:
        if ''.join(compreli[d]) in compreli:
            altro = ''.join(compreli[d])
            compreli[d] = compreli[altro] +  compreli[d]
    
    compre_finale = {}
    for d in compreli:
        if d in insi:
            compre_finale[d] = compreli[d]
    f = open(fout, 'w')
    json.dump(compre_finale,f)
    f.close()
