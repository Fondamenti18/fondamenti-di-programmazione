...
from io import open
def pianifica(fcompiti,insi, file):
    '''salva in formato json un dictionary avente per indice il numero dell'esercizio e per valore una lista contentente tutti gli esercizi da eseguire con precedenza'''
    dic, ls, i = {}, [], ''
    def genDic(i, res):
        ls.append(dic[i])
        while len(ls):
            for c in ls.pop():
                if c not in res:
                    ...
                    ls.append(dic[c])
        return res
    def filterAndSave(insi, file):
        with open(file, "w") as f: 
            z = {i : genDic(i, []) for i in dic if i in insi}
            dump(z, f)
    fs = getsize(fcompiti)
    b = bytearray(fs)
    with open(fcompiti, "rb", fs) as f: f.readinto(b)
    t = str.maketrans('subcomp\n', '        ')
    for l in b.decode('utf-8').splitlines():
        if not 'comp' in l: 
            dic[i] += [l.translate(t).strip()]
            continue
        i = l.translate(t).strip()
        dic[i] = []    
    filterAndSave(insi, file)