...
def caricadaticompitodafile(compitodatrovare):
    
    global testofile
    
    compitotrovato = False
    
    subcompiti = []
    
    for rigaletta in testofile:
        
        rigaletta = rigaletta.replace("\n", "").replace(" ", "")
       
        if rigaletta.startswith("comp"):
            
            if not compitotrovato:
                
                rigaletta = rigaletta.replace("comp", "")
                
                if int(compitodatrovare) == int(rigaletta):
        ...
        if rigaletta.startswith("sub"):
            
            if compitotrovato == True:
            
                rigaletta = rigaletta.replace("sub", "")
                       
                subcompiti.append(rigaletta)
    
    return subcompiti


def trovacompito(compito, listasubcompiti):
    
    global dizionariocompiti

    for subcompito in dizionariocompiti[compito]:
        
        trovacompito(subcompito, listasubcompiti)
        
        listasubcompiti.append(subcompito)
        
    return listasubcompiti
           
def pianifica(fcompiti,insi,fout):
    '''Implementare qui la funzione'''
    
    dizionariocompitirestituiti = {}
    
    global dizionariocompiti
    
    compiti = open(fcompiti)
    
    righefile = compiti.readlines()
    
    rigaletta = ""
    
    for riga in righefile:
        
        rigaletta = riga.replace("\n", "").replace(" ", "")
       
        if rigaletta.startswith("comp"):
                
            compitoletto = rigaletta.replace("comp", "")
                
            dizionariocompiti[compitoletto] = []
  
        if rigaletta.startswith("sub"):
            
            subcompitoletto = rigaletta.replace("sub", "")
            
            listatemporaneasubcompiti = []
            
            listatemporaneasubcompiti = dizionariocompiti[compitoletto]
            
            listatemporaneasubcompiti.append(subcompitoletto)
                
            dizionariocompiti[compitoletto] = listatemporaneasubcompiti
    
    for compitodatrovare in insi:
        
        if compitodatrovare in dizionariocompiti:
            
            dizionariocompitirestituiti[compitodatrovare] = trovacompito(compitodatrovare, [])
    
    with open(fout, 'w') as f:
    
        json.dump(dizionariocompitirestituiti, f)
