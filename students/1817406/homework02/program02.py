...
def pianifica(fcompiti,insi,fout):
    compiti={}
    compito=0
    with open(fcompiti, "r") as fi:
        for f in fi:
            if f.find("comp")!=-1:
                compito=int(re.findall('\d+',f)[0])
                compiti[compito]="none"
            else:
                compiti[compito]=re.findall('\d+',f)[0]
    print(compiti)
    finale={}
    for comp in insi:
        iterat=int(comp)
        ...
        try:
            while True:
                if compiti[iterat]=="none":    
                    break
                else:
                    fun.append(compiti[iterat])
                    iterat=int(compiti[iterat])
           
            finale[comp]= fun
        except:
             print("COMPITO", comp,"INESISTENTE")      
        
    print("RISULTATO" , finale)
    try:
        file = open(fout, "w") 
        file.write (str(finale).replace("'",'"'))
        file.close()
        print("FILE", fout, "SCRITTO")
    except:
        print("ERRORE NELLA SCRITTURA DEL FILE", fout)
