...
def pianifica(fcompiti,insi,fout):
    lstFile=Leggi(fcompiti)
    lstFile=AnalizeList(lstFile)
    risultato = Analize(lstFile,insi)
    write(fout,risultato)
...
def AnalizeList(lst):
    ret={}
    for el in lst:
        if el.strip().lower()[:4] == 'comp':
            ind = int(el.strip()[4:].strip())
            ret[ind] = []
        elif el.strip()[:3] == 'sub':
            if ind in ret.keys():   
                ret[ind]=(el.strip()[3:].strip())
    return ret

def Analize(diz,ins):
    ris={}
    for el in ins:
        if int(el) in diz.keys():
            ris[el]=[]
        else:
            continue
        find=int(el)
        lbTrovato=False
        while lbTrovato==False:
            
            if find in diz.keys() and len(diz[find])>0:
                ris[el].insert(0,diz[find])
                find = int(diz[find])
            else:
                lbTrovato=True
