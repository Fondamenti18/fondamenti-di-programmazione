...
def ripulisci(testo):
    #Rimuove le parti letterali dalle parole lasciando solo l'ID.
    testo_rip = ''
    for c in testo:
        if c.isdigit():
            testo_rip += c
...
def pianifica(fcompiti, insi, fout):
    ...
    diz = {}  #(Dizionario di appoggio)
    risultato = {} #(Dizionario risultato)
    
    with open(fcompiti, encoding = 'utf-8') as f:
        
        #Legge il file per righe, per ogni riga elimina tutti gli spazi.
        righe = [riga.strip().replace(' ','') for riga in f.readlines()]
        
        #Scorre la lista contenente le righe e compila un dizinario
        #di appoggio del tipo {compX : subY}.
        for indice, riga in enumerate(righe):
            
            if 'sub' in riga:
                diz[ripulisci(righe[indice-1])] = ripulisci(riga)
            else:
                diz[ripulisci(riga)] = ''
                
        #Scorre l'insieme con gli ID dei compiti da pianificare
        #e ad onguno associa i compiti preliminari.
        for ID in insi:
            ...
            if ID in diz.keys():
                ...
                if diz[ID] != '':
                    risultato[ID].append(diz[ID])
    
                for attr in risultato[ID]:
                    if attr in diz.keys() and diz[attr] != '':
                        risultato[ID].append(diz[attr])
                risultato[ID].reverse()
                
        #Crea il file .json contenente il dizionario risultato.      
