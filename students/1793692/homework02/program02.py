...
def pianifica(fcompiti,insi,fout):
    f=open(fcompiti, 'r',encoding ='utf-8').readlines()
    f.append('x')
    d={}
    for x in range(len(f)-1):
        i=x+1
        if 'comp' in f[x] and 'sub' in f[i]:
            d[tuttinumeri(f[x])]=tuttinumeri(f[i])
        elif 'comp' in f[x] and 'comp' in f[i]:
            d[tuttinumeri(f[x])]=[]
        elif 'comp' in f[x] and 'x' in f[i]:
            d[tuttinumeri(f[x])]=[]
    d2={}
    instot=set(d.keys())
    ins=set(instot&insi)
...
def tuttinumeri(testo): 
    for x in range(len(testo)):
        if not testo[x].isnumeric():
