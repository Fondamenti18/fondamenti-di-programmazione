...
def pianifica(fcompiti,insi,fout):
    ...
    fileLetto=file.read()
    fileSplit=fileLetto.splitlines()
    pila=dict()
    x=insi
    for v,k in enumerate(x):
        indice=k
        indiceDiAppoggio=indice 
        ris=[]
        sub=search(fileSplit,indice)
        if sub != -1:
            while sub != -1 and sub != None:   
                if sub != -1:
                    ris.append(sub)
                    indiceDiAppoggio = sub
                    sub = search(fileSplit, indiceDiAppoggio)
                elif sub==None and sub != -1:
                    pila.setdefault(indice,[])

                    
            ris.reverse()
            
            pila.setdefault(indice, ris) 
    file.close()
        
    with open(fout,'w') as file2: 
        data=json.dumps(pila)
        file2.write(data)
...
def search(lista,valore):
    for i in range(len(lista)):
        riga=lista[i].replace(' ','')
        
        if riga=='comp'+str(valore):
           if i == len(lista)-1:
               return None
           else:
               sottoriga=lista[i+1].replace(' ','')
               if sottoriga.find('sub') != -1:
                   x=sottoriga[3:]
                   return x
               else:
                   return None
