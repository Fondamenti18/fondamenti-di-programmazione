...
def check01(fcompiti):
    '''pulisce il testo eliminando la b e la p cosi da separare
    gli id dal tipo di compito con uno spazio'''
    file=open(fcompiti)
    cont=0
    for riga in file:        
        linea=riga.replace('p',' ').replace('b',' ').split() 
        if linea[0]=='com':
            dizionario[linea[1]]=[]
            cont=linea[1]
        elif linea[0]=='su':
            dizionario[cont].append(''.join(linea[1]))         
    file.close()


def togli(dizionario,insi):
    result=dict() 
    for x in insi:
        if x in dizionario:
            result[x]=dizionario[x]
    return result


def funzionefinale(dicto,dizionario):
    for k in dicto:
        if dicto[k]!=[]:
            for x in dicto[k]:                
                try:                    
                    dicto[k].append(''.join(dizionario[x]))
                except:
                    pass
        dicto[k]=dicto[k][::-1]
    for p in dicto.values():
        try:
            p.remove('')
        except:
            pass 
...
def pianifica(fcompiti,insi,fout):
    check01(fcompiti)     
    dicto=togli(dizionario,insi)
    risultato=funzionefinale(dicto,dizionario)
