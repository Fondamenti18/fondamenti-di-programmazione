...
def pianifica(fcompiti,insi,fout):
    ...
    fcompiti=open(fcompiti,'r')
    Testo=fcompiti.read()
    NTesto=SistemTesto(Testo)
    Elementi=[]
    Elementi=Smista(NTesto)
    dic = ritorna(Elementi)
    #Stampa=stampa(dic,insi,fout)
    #return Stampa
    return stampa(dic,insi,fout)
    #Stampa=stampa(dic,insi,fout)
    #return Stampa
              
def SistemTesto(Testo):
    Testo=Testo.split('\n')
    NTesto=[]
    for elemento in Testo:
           NTesto.append(elemento.replace(' ',''))
    #NTesto=NTesto[:-1]
    return NTesto
...
def ritorna(NTesto):
       dic={}
       for elemento in NTesto:
              if len(elemento)==2:
                     dic[elemento[0][4:]]=elemento[-1][3:]
              elif len(elemento):
                     dic[elemento[0][4:]]=''
       return dic

def stampa(dic,insi,fout):
       insieme={}
            ...
            if elemento in dic:
                   ...
                   while val:
                          ...
                          if val:
                                 ins.append(val)
                   ins.reverse()
                   insieme[elemento]=ins
       with open(fout,'w') as a:
              json.dump(insieme,a)          
def Smista(NTesto):
       c=[]
       indice=0
       while indice!=len(NTesto)-1:
              if indice==len(NTesto)-2:
                     c+=[[NTesto[indice]]]
                     return c
                     
              else:
                     if NTesto[indice][0]=='c' and NTesto[indice+1][0]=='s':
                            c+=[[NTesto[indice],NTesto[indice+1]]]  
                            indice+=1
                     elif NTesto[indice][0]=='c' and NTesto[indice+1][0]=='c':
                            c+=[[NTesto[indice]]]
                            indice+=1  
                     elif NTesto[indice][0]=='s' and NTesto[indice+1][0]=='':
                            c+=[[NTesto[indice]]]
                            indice+=1     
                     else:
                            indice+=1
