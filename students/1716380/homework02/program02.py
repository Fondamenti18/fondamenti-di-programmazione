...
def pianifica(fcompiti,insi,fout):
    ...
    dic=jala(fcompiti,insi)
    with open(fout,'w',encoding='utf-8') as fj:
        json.dump(dic,fj)

def conta(fcompiti,w):
    lis=nf(fcompiti)
    compp=comp(lis)
    end=''
    if w in compp:
        if ['comp',w]==lis[-1]:
            ...
            position=lis.index(['comp',w])
            nextline_first_element=lis[position+1][0]
            nextlinenum=lis[position+1][1]
            if nextline_first_element=='sub':
                end+=nextlinenum
    else:
        end+='0'
    return end
...
def nf(fcompiti):
    lista=[]
    with open(fcompiti,'r') as fo:
        for i in fo:
            x=i.split()
            if len(x)==1 and x[0][:3]=='sub':
                x.append(x[0][3:])
                x[0]=x[0].replace(x[0][3:],'')
            if len(x)==1:
                x.append(x[0][4:])
                x[0]=x[0].replace(x[0][4:],'')
            lista.append(x)
    return lista
