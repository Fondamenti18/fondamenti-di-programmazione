...
def pianifica(fcompiti,insi,fout):
    ...
    import re
    import json
    with open(fcompiti, 'r') as f:fcompiti=f.read()
    dizionario={}
    

    fcompiti=" ".join(fcompiti.split())
    fcompiti = fcompiti.replace(' ', '')
    variabile=re.split('(\d+)',fcompiti)
    variabile.pop()
    copia=variabile[:]
    ...
    for x in variabile:
        if x=="comp":
            i=copia.index(x)
            dizionario[variabile[i+1]]=[]
            copia.remove(x)
            copia.insert(i,"*")
        if x=="sub":
            ii=copia.index(x)
            dizionario[variabile[i+1]]=[variabile[ii+1]]
            copia.remove(x)
            copia.insert(ii,"*")
    ...
    def check(x,dizionario):    
        global lista
        appoggio=dizionario[x]
        if appoggio:
            val=appoggio[0]
            
            if val:
                lista.append(val)
                check(val,dizionario)
            else:
                 return lista
    ...
    for x in insi:
        ...
        if x in dizionario:                
            check(x,dizionario)
            dizionario2[x]=lista[::-1]
    
    
    with open(fout, 'w') as fp:
        json.dump(dizionario2,fp)
        
    '''json=json.dumps(dizionario2) 
    f=open('a.json', 'w') 
    f.write(json)
    f.close'''
