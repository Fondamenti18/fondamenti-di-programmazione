...
def pianifica(fcompiti,insi,fout):
    '''
    Il dizionario seguente contiene l'associazione
    tra ognuno dei compiti elencati nell'insieme in 
    input "insi" e gli identificativi dei compiti
    preliminari.
    '''
    dictOut = {}
    '''
    Apro il file "fcompiti" e leggo in successione
    tutte le righe per popolare il dizionario "dictOut"
    con tutte le associazioni indicate nel file.
    '''
    comp_in_insi = 0
    comp=''
    fpIn = open(fcompiti,'r')
    for line in fpIn:
        if 'comp' in line:
            comp = line.replace(' ','').replace('\n','').replace('comp','')
            if comp in insi:
                dictOut[comp] = []
                comp_in_insi = 1
            else:
            ...
            if comp_in_insi == 1:
                ...
                dictOut[comp].append(sub)
    with open(fout,'w') as fpOut:
        json.dump(dictOut, fpOut)