...
def pianifica(fcompiti, insi, fout):
	import json
	f = open(fcompiti,'r').read().replace(' ','').replace('\n','') 
	c = f.split('comp')[1:]
	
	# creo diz d1 con a sx il compito e a dx il sub
	d1 = {}
	for e in c :
		l = e.split('sub')
		if len(l) == 2 :
	...
	# creo diz d2 con a sx il compito e a dx le liste
	d2 = {}
	for e in d1.keys() :
		if e in insi :
			g = d1[e]
