...
def pianifica(fcompiti,insi,fout):
        ...
        diz={}
        dizfinale={}
        data=file.read().replace(" ","").replace('comp','comp ').replace('sub','sub ').splitlines()
        i=0
        while i<=len(data)-1:
                if i!=len(data)-1:
                        if 'sub' in data[i+1]:
                                diz[data[i].replace('comp ','')]=data[i+1].replace('sub ', '') 
                                i+=2
                        else:
                                diz[data[i].replace('comp ','')]=""       
                                i+=1
                else:
                        diz[data[i].replace('comp ','')]=""
                        i+=1
