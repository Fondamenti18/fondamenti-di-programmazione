...
def ai(griglia_corrente, griglia_precedente, x, y, vx, vy, car, verso, startx, starty, laps):
    if destra == True:
        if griglia_corrente[y][x+sum(list(range(vx+2)))] == ' ' or griglia_corrente[y][x+sum(list(range(vx+2)))] == '|':
            return +1,0
        elif griglia_corrente[y][x+sum(list(range(vx+1)))] == ' ' or griglia_corrente[y][x+sum(list(range(vx+1)))] == '|':
            return 0,0
        else:
            return -1,0
