...
def pianifica(fcompiti,insi,fout):
    ...
    for i in file:
        fileunito+=i
    fileunito=fileunito.replace(' ','')
    listacomp=fileunito.split('\n')
    if '' not in listacomp:
        listacomp+=['']
    comp={}
    for i in listacomp:
        if 'sub' not in i and i!='':
            sub=''
            if 'sub' in listacomp[listacomp.index(i)+1]:
                sub+=listacomp[listacomp.index(i)+1]
            comp[i]=sub
    idcomp={}
    for i in comp:
        chiave=i[::-1]
    ...
    for i in insi:
        if i in idcomp:
            if idcomp[i]=='':
                ...
                y=idcomp[i]
                lista=[y]
                while y in idcomp and idcomp[y]!='':
                    lista+=[idcomp[y]]
                    y=idcomp[y]
            lista.reverse()
            risultato[i]=lista
