...
def pianifica(fcompiti,insi,fout):
      with open(fcompiti, 'r',encoding='utf-8') as file:
            ogg={}
            ris={}
            ogg=oggetto(file,ogg)
            
            for i in insi:
                  l=[]
                  ris=risultato(ris,i,ogg,l)
                        
      with open(fout, 'w',encoding='utf-8') as ft:
            r=str(ris).replace("'", '"')
            ft.write(r)

def risultato(ris,i,ogg,l):
      if i in ogg.keys():
            j=i
            while ogg[i]!=[]:
                  if str(ogg[i]) in ogg.keys():
                        l.append(ogg[i])
                        i=ogg[i]
            l.reverse()
            ris.setdefault(j,l)
