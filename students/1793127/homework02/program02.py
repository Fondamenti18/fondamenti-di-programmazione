...
def pianifica(fcompiti,insi,fout):
	with open(fcompiti) as f:
		...
		testo=testo.replace(' ','')
		testo=testo.replace('\n',' ')
		lista_compiti=testo.split(' ')
		cont=0
		for x in lista_compiti:
			if x[:4]=='comp':
				if lista_compiti[cont+1][:3]=='sub':
					diz[x]=lista_compiti[cont+1]
				else:
					diz[x]='nosub'
			cont+=1
		for chiave in diz:
			lista_sub=[]
			if chiave[4:] in insi and diz[chiave]!='nosub':
				lista_sub.append(diz[chiave][3:])
				next='comp'+diz[chiave][3:]
				while diz[next]!='nosub':
					lista_sub.append(diz[next][3:])
					next='comp'+diz[next][3:]
				dati[chiave[4:]]=lista_sub[::-1]																							
			if chiave[4:] in insi and diz[chiave]=='nosub':
				dati[chiave[4:]]=lista_sub				
