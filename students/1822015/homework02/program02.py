...
def pianifica(fcompiti,insi,fout):
	diz = {}
	wLine = []
	with open(fcompiti, encoding='utf8') as file:
		for line in file:
			line = line.rstrip()
			for c in line:
				if c in '0123456789':
					...
					break
			wLine += line.split()
		#dovrei elaborare solo i comp/sub che sono presenti in insi
		for i, val in enumerate(wLine):
			if val == 'comp':
				diz[wLine[i+1]] = []
			if val == 'sub':
				diz[wLine[i-1]] = [wLine[i+1]]
				
	#ora ho creato il dizionario con tutti i COMP SUB relativi al file preso in esame
	# ora devo cercare di aggiungere ad ogni valore del dizionario tutti i COMP collegati ai SUB
	
	for id, val in diz.items():
		for v in val:
			diz[id] = diz.get(v) + diz.get(id)
	
	newdiz = {}
	for i in insi:
		if i in list(diz.keys()):
			newdiz[i] = diz.get(i)
	...
	#potrei leggere tutte le righe e mettere in un dizionario tutti i COMP con relativo SUB associato come valore
	#per ogni SUB aggiungo all valore della CHIAVE l'insieme di tutti gli altri elementi della CHIAVE uguale a SUB
	
	#poi potrei intersecare i due dizionari
