...
def generaMap(righe,map):
	...
	for i in range(0, len(righe)):
		...
		if (riga.startswith("c")):
			numeroCorrente=riga[4:]
			if passo0==1:
				passo0=0
				numeroPrecedente=numeroCorrente
			else:
				
				map[numeroPrecedente]=lista
				lista=[]
				numeroPrecedente=numeroCorrente
...
def elabora(listaappo,map,lista): 
	
	for i in range(0, len(listaappo)):
		numAppo=listaappo[i]
		lista=elabora(map[numAppo],map,lista)
...
def pianifica(fcompiti,insi,fout):
	...
	lista=[]
	diz={}
	map=generaMap(righe,map)
	for y in range(0,len(listInsi)):
		num=listInsi[y]
		if num in map.keys():
			diz[num]=elabora(map[num],map,lista)
			lista=[]	
	f = open(fout, "w") 
