...
def trova_attr_class(nodo):
    ...
    return lista
def t_t_n(nodo,selettore):
    ins=set()
    if selettore in nodo.tag:
        ins=ins.union(set([nodo]))
    for figlio in nodo.content:
        if type(figlio)!=str:
            ins=ins.union(set(t_t_n(figlio,selettore)))
    return ins



def select(selettore):
...
def t_a_v(nodo,selettore,lista):
    chiave_sel=select(selettore)[0]
    valore_sel=select(selettore)[1]
...
def conta_nodi(fileIn, selettore):
    ...
    if selettore[0] not in '.#@' and '>' not in selettore:
        ...
        for x in range(lun):
            for nodo in lista:
                esito+=list(t_t_n(nodo,selettori[x]))
                lista.remove(nodo)
            lista+=esito
        for el in lista:
    ...
    if '>' in selettore:
        selettori=selettore.split(' > ')
        lista1=list(t_t_n(pag,selettori[0]))
        cont=0
        for el in lista1:
