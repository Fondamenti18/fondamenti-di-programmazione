...
def pianifica(fcompiti,insi,fout):
    ...
    precedenti = collect(fcompiti)
    piano = plan(precedenti, insi)
    with open(fout, 'w') as o:
        json.dump(piano, o)
        #json.dump(piano, o, sort_keys=True, indent=4)

def collect(fcompiti):
    cerca = r"comp *([0-9]+)|sub *([0-9]+)"
    cerca = re.compile(cerca)
    precedenti = {}
    cid = None
    with open(fcompiti) as f:
        for line in f:
            match = re.search(cerca, line)
            if match:
                post, pre = match.groups()
                if post:
                    cid = post
                    precedenti[cid] = None
                elif pre:
...
def plan (compiti, cercati):
    risultato = {}
    for c in cercati:
        if c in compiti:
            risultato[c] = []
            precedente = compiti[c]
            while precedente:
                risultato[c].insert(0,precedente)
                precedente = compiti[precedente]
    return risultato


if __name__ == '__main__':
    print(pianifica('file02_10_2.txt', { '1', '2', '3', '4', '5', '6', '7', '8', '9' }, 'prova.json'))
