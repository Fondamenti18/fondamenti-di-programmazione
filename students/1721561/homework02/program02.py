...
def pianifica(fcompiti,insi,fout):
    with open(fcompiti, mode="r", encoding="utf-8") as file_being_read:
        all_ids = set()
        curr_id = "0"
        prev_id = "0"
        dep_tasks_id = set()
        ...
        for line in file_being_read:
            prev_id = curr_id
            curr_id = "".join(filter(str.isdigit, line))
            all_ids.add(curr_id)
            if "sub" in line:
                dep_tasks_id.add(prev_id)
                dependencies.append((prev_id, curr_id))
        
        
        indep_tasks_id = all_ids - dep_tasks_id
        #print ( indep_tasks_id)
        #print (dependencies)
        ...
        for value in d.values():
        	value = value.reverse()
        for task_id in indep_tasks_id:
        	d[task_id]=[]
        #print (d)
        actual_insi = all_ids & insi
        for key in (set(d.keys()) - actual_insi):
        	del d[key]
        #result = dict.fromkeys(insi, [])
    with open(fout, mode="w", encoding="utf-8") as file_being_written:
    	json.dump(d, file_being_written)
