...
def pianifica(fcompiti,insi,fout):
    openfile = open(fcompiti,'r')
    text = openfile.read()    
    new_text= ''
    for x in text:
        if x.isalpha() or x == '\n' or x.isdigit():
            new_text += x
    lista01 = new_text.split(sep='\n')
    
    elenco = {}
    for x in range(len(lista01)):
        if 'comp' in lista01[x]:
            if 'sub' in lista01[x+1]:
                elenco[compID(lista01[x])] = [lista01[x],lista01[x+1]]
            else:
                elenco[compID(lista01[x])] = [lista01[x]]
    diz = {}
    for x in insi:
        a=x
        b=x
        lista = []
        if a in elenco.keys():
            while len(elenco[a]) == 2:
                lista.append(compID(elenco[a][1]))
                a = compID(elenco[a][1])
            diz[b] = [x for x in reversed(lista)]
