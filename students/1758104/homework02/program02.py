...
def pianifica(fcompiti, insi, fout):
    with open(fcompiti, 'r') as file:
        file = file.readlines()
    l = []
    #controllo il tipo di compito e lo salvo in una stringa s
    for riga in file:
        if 'comp' in riga:
            s = 'comp'
        else:
            s = 'sub'
        #per trovare id splitto comp1
        id = riga.split(s)[1]
        id = id.strip()
        t = (s, id)
        #creo lista di tuple (tipo, id)
        l.append(t)

    dip = set()
    indip = set()
    dipendenze = {}
    for i, tupla in enumerate(l):
        if i + 1 >= len(l):
            if 'comp' in tupla[0]:
            #controllo se è indipendente
                indip.add(tupla[1])
        else:
            if 'comp' in tupla[0]:
            #controllo se è dipedente e da chi
                if l[i + 1][0] == 'sub':
                    dip.add(tupla[1])
                    dipendenze[tupla[1]] = l[i+1][1]
                else:
                    indip.add(tupla[1])
            else:
                continue
    ...
    for comp in insi:
        ...
        if copy in dip:
            if copy != comp and copy not in ris:
                ris.insert(0, copy)
            while True:
                if dipendenze[copy] in dipendenze.keys():
                    if dipendenze[copy] not in ris:
                        ris.insert(0, dipendenze[copy])
                    copy = dipendenze[copy]
                    continue
                else:
                    if dipendenze[copy] not in ris:
                        ris.insert(0, dipendenze[copy])
