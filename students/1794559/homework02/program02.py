...
def cerca(el,dove,eccolo):
	try:
		eccolo.append(str(dove[int(el)]))
		return cerca(str(dove[int(el)]),dove,eccolo)
	except KeyError:
		pass
...
def pianifica(fcompiti,insi,fout):
	...
	with open(fcompiti)as f:
		lines=f.read()
		lines=lines.split("comp")
		for i in lines:
			aa.append([int(x.group()) for x in re.finditer(r'\d+', i)])
	...
	for i in insi:
		if int(i)in eh:
			lista=[]
			cerca(int(i),di,lista)
			dizionario[i]=list(reversed(lista))
