...
def pianifica(fcompiti,insi,fout):
    ...
    fcompiti=open(fcompiti,"r").read()
    d=diz(fcompiti)
    return verifica(d,insi)
   
    
    
            
            
def verifica(dic,insi):
       insieme={}
       for elemento in insi:
            li=[]
            if elemento in dic:
                li.append(dic[elemento])
                insieme[elemento]=li
                for el in dic:
                    if list(el) == insieme[elemento]:
                        insieme[elemento].extend(dic[el])
...
def diz(x):
    ...
    dizionario={}
    for elem in l:
        dizionario.update({elem[0]:elem[1]})
    return dizionario
...
def stampa(dic,insi,fout):
       ...
       with open(fout,'w') as a:
              json.dump(insieme,a)
       #return insieme #da togliere dopo i test    
