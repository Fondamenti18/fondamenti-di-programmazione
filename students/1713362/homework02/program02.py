...
Gli (ID) devono comparire nella lista nell'ordine di esecuzione corretto, dal primo fino a quello
precedente a quello richiesto (ovviamente il primo ID di una lista non vuota corripondera' sempre
ad un compito che non richiede un compito preliminare).
...
def pianifica(fcompiti,insi,fout):
    dict = {}
    with open(fcompiti,'r',encoding = 'utf-8') as file:
        l = file.readlines()
    lista_tuple = []
    id_in_file = []
    for processo in l:
        p = processo.strip()
        lista_tuple.append(getProcessInfo(p))
        id_in_file.append(lista_tuple[-1][1])
    for elem in insi:
        if elem in id_in_file:
            process_index = 0
            for p in lista_tuple:
                if p[1] == elem and p[0] == 'comp':
                    process_index = lista_tuple.index(p)
                    break
            dict[elem] = getProcessList(lista_tuple, process_index)
    with open(fout, 'w') as f:
        json.dump(dict,f,ensure_ascii=False)


def getProcessList(lista, indice):
    i = indice
    return_list = []
    while lista[i+1][0] != 'comp':
        return_list.insert(0,lista[i+1][1])
        for e in lista:
            if e[1] == lista[i+1][1] and e[0] == 'comp':
                i = lista.index(e)
                break
        if i+1 >=len(lista):
            break
...
def getProcessInfo(s):
    ...
    index = 0
    while index < len(s):
        if s[index] in numeri:
            id += s[index]
        elif s[index] in alfabeto:
            type += s[index]
        index += 1
    return (type,id)
