...
import json
def diz_comp_sub(testo):
    with open(testo,encoding='utf-8')as f:
        linea=f.read()
        linea=linea.replace('\n','')
        linea=linea.replace('sub',' ')
        linea=linea.split('comp')
        compiti=[]
        sub=[]
        for x in linea:
            f=x.split()
            if f ==[]:
                 ...
                 compiti.append(f[0])
                 sub.append([f[1]])
        return dict(zip(compiti,sub))
       
def pianifica(fcompiti,insi,fout):
    dizionario=diz_comp_sub(fcompiti)    
    diz_risultato=dict()
    for k,v in dizionario.items():
         if k in insi:
             ...
             while v!= []: 
                 for x in v:
                     valori=dizionario[x]+valori
                     v=dizionario[x]
             diz_risultato[k]=valori
    with open(fout,mode='w',encoding='utf-8') as d:
        json.dump(diz_risultato,d)
