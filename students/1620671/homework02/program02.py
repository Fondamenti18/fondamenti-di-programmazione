...
def pianifica(fcompiti,insi,fout):
    testo = testo_s(fcompiti)
    d = dict()
    diz_ris = dict()
    d = sistema(dizionario_base(d, testo), insi)
    diz_ris = dizionario_ris(d, insi)
    with open(fout, 'w') as of:
            json.dump(diz_ris, of, ensure_ascii = False)

def testo_s(fcompiti):
    with open(fcompiti, encoding = 'utf-8') as f:
        testo = f.read()
        testo = testo.splitlines()
    return testo

def dizionario_base(d, testo):
    for i in range(len(testo)):
        if 'comp' in testo[i] and str(int(testo[i][testo[i].index('p')+1:])) not in d:
            d[str(int(testo[i][testo[i].index('p')+1:]))] = []
        else:
            d[str(int(testo[i-1][testo[i-1].index('p')+1:]))] += [str(int(testo[i][testo[i].index('b')+1:]))]
    return d

def dizionario_ris(d, insi):
    diz_ris = dict()
    for i in insi:
        if i in d:
            diz_ris[i] = d[i]
