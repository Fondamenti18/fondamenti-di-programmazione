...
def conv(n):
    if n==0:
        ...
        decineDaVentiaNovanta=("venti","trenta","quaranta","cinquanta","sessanta","settanta","ottanta","novanta")
        numeroAggiuntivo=decineDaVentiaNovanta[int(n/10)-2]
        NS=n%10
        if NS==1 or NS==8:
            numeroAggiuntivo=numeroAggiuntivo[:-1]
        return numeroAggiuntivo+conv(n%10)
    elif n<=199:
        return "cento"+conv(n%100)
    elif n<=999:
        MS=n%100
        MS=int(MS/10)
        numeroAggiuntivo="cent"
        if MS!=8:
            numeroAggiuntivo=numeroAggiuntivo+"o"
        return conv(int(n/100))+numeroAggiuntivo+conv(n%100)
    elif n<=1999:
