...
HOLE = 'O'
PLAYER = 'X'
OPPONENTS = list(string.ascii_uppercase)
GOAL = '|'
FREE = ' '
...
def pathfinder(grid, start, goal, direction):
    ...
    while not frontier.empty():
        ...
        for next_cell in get_neighbors(grid, *current):
            if get_cell(grid, *next_cell) == FREE:
                new_cost = cost_so_far[current] + 1
                if next_cell not in cost_so_far or new_cost < cost_so_far[next_cell]:
                    cost_so_far[next_cell] = new_cost
                    priority = new_cost + heuristic_distance(goal, next_cell)
...
def ai(griglia_corrente, griglia_precedente, x, y, vx, vy, car, verso, startx, starty):
    ...
    if x == startx and y == starty:
        path = pathfinder(griglia_corrente, (x, y), (startx - verso * 2, starty), verso)
        path.append((startx - verso, starty))
