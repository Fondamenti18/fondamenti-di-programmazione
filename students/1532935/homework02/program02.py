...
def find_id(idtest,allword):
      
      str_comp='comp'+str(idtest)
      if (str_comp +'c' in allword  ):
            return  idtest
      elif ( str_comp  in allword[-len(str_comp):] ):
            return  idtest
      elif('comp'+str(idtest) +'sub' in allword ):
            strfind='comp'+str(idtest) +'sub'
            indx_strfind=allword.find(strfind)
            strresult=allword[indx_strfind+len(strfind):indx_strfind+len(strfind)+6]
            numfind=''
            for i in strresult:
                  if(i=='c'):
                        break
                    ...
                    numfind=numfind+i
            return  idtest+','+find_id(numfind,allword)
      else:
            return ''
...
def pianifica(fcompiti,insi,fout):
    ...
    outdict={};
    #Read File and Save massage array
    f = open(fcompiti,'r',encoding='utf8')
    message = f.read()
    f.close()

    # Frist AllWord Extract and Initial Parameter
    newallword=message.replace(" ", "").replace('\n', '')



    for idtest in insi :
        result=find_id(idtest,newallword)
        result=result.strip()

        if len(result)==len(idtest):
            outdict[str(result)]=[]
        else:
            selid=result[len(idtest)+1:].split(',')
            selid=[x for x in selid if x]
            selid=selid[::-1]
            if selid:
                outdict[result[0:len(idtest)]]=selid
