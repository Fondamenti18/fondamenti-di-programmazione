...
def conv(n):
    if n == 0:
        ...
        parolanum = decine[int(n/10)-2]
        elisione = n%10
        if elisione == 1 or elisione == 8:
            parolanum = parolanum[:-1]
        return parolanum + conv(n % 10)
    elif n <= 199:
        return "cento" + conv(n%100)
    elif n <= 999:
        elisione2 = n%100
        elisione2 = int(elisione2/10)
        parolanum = "cent"
        if elisione2!=8:
            parolanum = parolanum + "o"
        return conv(int(n/100)) + parolanum + conv(n%100)
    elif n <= 1999:
