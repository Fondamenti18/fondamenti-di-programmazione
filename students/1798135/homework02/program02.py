...
def pianifica(fcompiti,insi,fout):
	with open(fcompiti) as file:
		...
		for riga in file.readlines():
			k=re.search("(comp)+(\s)*(\d+)",riga)
			#print(k)
			if k:
				key=k.group(3)
				lettura[key]="-1"
			else:
				k=re.search("(sub)+(\s)*(\d+)",riga)
				#print(k)
				if k:
					lettura[key]=k.group(3)
		for i in insi:
			p=[]
			flag=True
			var=i
			while(flag):
				if var in lettura.keys():
					p+=[lettura[var]]
					var=lettura[var]
				else:
					flag=False
...
def ricerca(f,n):
	...
	#cerca comp+spazio+numero+jolly+newline+sub+spazio+numero
	k=re.search("(comp)+(\s*)"+n+"\\b(.*)(\n)+(.*)(sub)(\s*)(\d+)",f,re.MULTILINE)
	
	if k:
		res=k.group(8)
		return [res]+ricerca(f,k.group(8))
	elif re.search(n+"\\b",f):
		return [-1]
	else:
		return [-2]"""
