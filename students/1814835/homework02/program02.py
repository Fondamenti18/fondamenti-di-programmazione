...
#insi =  {'2','4','11','1','6','9','10'}
#fout = 'test1.json'
import json
#import re
#lista.append(re.findall('\d+|\D+',testo))
def pianifica(fcompiti,insi,fout):
    ...
    D = {}
    for s in testo2:
        lettere = s.rstrip('0123456789')
        numeri = s[len(lettere):]
        testo3.append(lettere)
        testo3.append(numeri)
    for el in range(len(testo3)):
        if testo3[el] != '':
            testo4.append(testo3[el])
    for i in insi:
        if i in testo:
            for el in range(len(testo4)):
                if testo4[el] == 'comp':
                    d[testo4[el+1]] = []
                    if el <= (len(testo4)-4):
                        if testo4[el+2] == 'comp':
                            d[testo4[el+1]] = []
                        if testo4[el+2] == 'sub':
                            if (el + 2) < (len(testo4)-1):
                                d[testo4[el+1]] = [testo4[el+3]]
                            #if (el + 4) < (len(testo4)-1):
                            #    d[testo4[el+1]] = [testo4[el+3],testo4[el+5]]
        if i in d.keys():
            D[i] = d[i]
            if str(d[i]) in d.keys():
                D[i] = [d[i],d[x]]                          
           # D[i] = d[i]
    print (d)
    print (D)
    with open(fout, 'w') as f_json:
        json.dump(D,f_json)
