...
class NodoTris:
    ...
    def getStrategiaVincente(self, player):
        if self.lista_figli == []:
            tipo = self.tipo()
            # print(tipo, giocatore)
            if tipo == player: return 1
            if tipo == '-': return 0
            return -1
        if self.turno != player:
            weight = 1
            for child in self.lista_figli:
                weight = min(weight, child.getStrategiaVincente(player))
        else:
            weight = -1
            for child in self.lista_figli:
                # child.turno = 'x' if giocatore=='o' else 'o'
                weight = max(weight, child.getStrategiaVincente(player))
        return weight
...
def checkVertical(griglia, simbol):
    # print(griglia,simbol)
    for i in range(3):
        # print(griglia[i],griglia[i+3], griglia[i+6])
