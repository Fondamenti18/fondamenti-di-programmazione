...
def lcompsub(dizionario,lista,ident):
    v = dizionario[ident]
    lista.append(ident)
    if v == []:
        return lista
    else:
        lcompsub(dizionario,lista,v)
        return lista
...
def pianifica(fcompiti,insi,fout):
    ...
    with open(fcompiti) as f:
        compiti = f.read()
    righe = compiti.splitlines()
    diz = {}
    dizuff = {}
    for i in range((len(righe)-1)):
        if 'comp' in righe[i] and 'sub' in righe[i+1]:
            diz[righe[i]] = righe[i+1]
        elif 'comp' in righe[i] and 'comp' in righe[i+1]:
            diz[righe[i]] = '0'
    ...
    drisultato = {}
    for el in insi:
        if el in dizuff.keys():
            lpercorso = lcompsub(dizuff,[], el)
            lpercorso.remove(lpercorso[0])
            lpercorso.reverse()
            drisultato[el] = lpercorso

    with open(fout, mode='w') as fjson:
        json.dump(drisultato, fjson)
