...
def pianifica(fcompiti,insi,fout):
    txt = open(fcompiti)
    lines = txt.readlines()
    scrolled = scroll(lines)
    updated = update(scrolled,insi)
    write(updated,fout)
    ...
    for i,line in enumerate(lines):
        if(is_comp(line)):
            try:
                ...
                next_line = ""
            id_1 = get_id(line)
            id_2 = get_id(next_line)

            if(is_comp(next_line) or next_line == ""):
                orders.update({id_1:[]})
            else:
                orders.update({ id_1:id_2 })
    return orders

def update(scrolled,insi):
    subs = None
    val = None
    new_set = {}
    final_set = {}

    for k in scrolled.keys():
        if(k not in insi): continue
        subs = []
        val = scrolled[k]
        if( val != []):
            subs.append(val)
        while (True):
            try:
                ...
                break
        subs.reverse()
        new_set.update({k : subs})
        final_set.update({k : subs})
    return final_set
