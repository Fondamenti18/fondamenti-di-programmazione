...
def conv(n):
    ...
    if n == 0:
        ...
        return ("uno", "due", "tre", "quattro", "cinque", "sei", "sette", "otto", "nove", "dieci", "undici", "dodici", "tredici", "tredici", "quattordici", "quindici", "sedici", "diciassette", "diciotto", "diciannove")[n-1]
    elif n<=99:
        decine=("venti", "trenta", "quaranta", "cinquanta", "sessanta", "settanta", "ottanta", "novanta")
        let= decine[int(n/10)-2]
        x=n%10
        if x==1 or x==8:
            let = let[:-1]
        return let + conv(n%10)
    elif n<=199:
        ...
        if m!=8:
            let=let+"o"
        return conv(int(n/100))+let+conv(n%100)
    elif n<=1999:
