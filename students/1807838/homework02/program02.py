...
def pianifica(fcompiti,insi,fout):
	...
	with open(fcompiti,encoding='utf-8') as f:
			...
			while a!='':
				li=[]
				cod=re.findall('[0-9]{1,6}',a)[0]
				if 'comp' in a:
					suc=f.readline()
					if 'sub' in suc:
						val=re.findall('[0-9]{1,6}',suc)[0]#valore del sub
						li.insert(0,val)#aggiungilo
						a=f.readline()
					else:
						a=suc
					d[cod]=li
	for x in d:#SCORRI IL DIZIONARIO PER INSERIRE I VALORI MANCANTI
		if x in insi:
			if d[x]!=[]:
				while d[d[x][0]]!=[]:
					d[x]=d[d[x][0]]+d[x]
	...
	for x in d:
		if x in insi:
			d1[x]=d[x]
	with open(fout,'w',encoding='utf-8') as f: dump(d1,f)
#pianifica('file02_10000_50.txt',{'10','20','30','40','50','60','70','80'},'test2.json')
