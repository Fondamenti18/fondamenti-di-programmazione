...
def pianifica(fcompiti,insi,fout):
    ...
    dictionary={}
    list_comp=make_list(fcompiti)

    for comp in list_comp:
        comp=comp.split('sub')
        if len(comp)==1:
            dictionary[comp[0].strip()]=[]
        else:
            dictionary[comp[0].strip()]=comp[1].strip()

    dictionary=organize_dict(dictionary,insi)
    save(dictionary,fout)
...
def make_list(path):
    ...
    with open(path,'r') as f:
        list_comp=f.read().strip().split('comp')

    return list_comp[1:]

def organize_dict(d,s):

    return_dict={}

    for element in d:
        if element in s:
            return_dict[element]=lister(d,element)
...
def lister(d,n):
    ...
    if d[n]!=[]:
        return_list=[d[n]]+return_list
        return_list=lister(d,d[n])+return_list
