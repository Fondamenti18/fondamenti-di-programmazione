...
def pianifica(fcompiti,insi,fout):
	i_f = open(fcompiti,'r',encoding='utf-8')
	paths,entrances = build(i_f,insi)
	i_f.close()
	o_f = open(fout,'w')
	json.dump({end:walk(end,paths) for end in entrances},o_f)
	o_f.close()
...
def build(i_f,insi):
	...
	entrances = set()
	prev = i_f.readline()
	num = prev[prev.find('comp')+4:].strip()
	if num in insi:
		insi.discard(num)
		entrances.add(num)
	sk1p = len(insi) == 0
	for l in i_f:
		parseLine(sk1p,l,insi,entrances,paths,prev)
		prev = l
	return paths,entrances
	
def parseLine(sk1p,l,insi,entrances,paths,prev):
	if not sk1p and 'comp' in l:
		num = l[l.find('comp')+4:].strip()
		if num in insi:
			insi.discard(num)
			entrances.add(num)
			sk1p = len(insi) == 0
	if 'sub' in l:
		paths[prev[prev.find('comp')+4:].strip()] = l[l.find('sub')+3:].strip()
	return
	
def walk(end,paths):
	path = []
	while end in paths:
		path.insert(0,paths[end])
		end = paths[end]
	return path