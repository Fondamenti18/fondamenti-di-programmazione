...
def pianifica(fcompiti,insi,fout):
	compiti = CreaDict(fcompiti)
	cambiamenti = 1 
	while cambiamenti:
		compiti, cambiamenti = OrdinaDict(compiti, 0)
	salva = dict()
	keys = compiti.keys()
	for i in insi:
		if i in keys:
	...
	with open(fout, 'w') as outfile:
		json.dump(salva, outfile)
	
def CreaDict(fcompiti):
	compiti = dict()
	for line in open(fcompiti):
		line = line.strip()
		if line[0] == 'c':
			keys = line[4:].strip()
			compiti[keys] = []
		elif line[0] == 's':
			compiti[keys] = [line[3:].strip()]
	return compiti
				
def OrdinaDict(compiti, cambiamenti):
	for key in compiti.keys():
		if compiti[key] and compiti[compiti[key][0]]:
			compiti[key] = compiti[compiti[key][0]] + compiti[key]
			cambiamenti += 1
	return compiti, cambiamenti
