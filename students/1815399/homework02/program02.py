...
def pianifica(fcompiti,insi,fout):
 f=open(fcompiti)
 compiti=f.readlines()
 compiti=rimuovi(compiti)
 insi= list(insi)
 
 compiti=splittamento(compiti,'comp')
 
 d=chiavi(compiti,insi)
 compiti=splittamento(compiti,'sub')
...
def rimuovi(lis):
    ...
    lista=[]
    while i < l:
        lista.append(lis[i][:-1])
        lista[i]= lista[i].strip()
        i+=1
    return lista    

def chiavi(lis,insieme):
     j=0
     lun=len(lis)
     while j < lun:
         y=0
         while y < len(insieme):
             if lis[j] == insieme[y]:
                 if lis[j+1][:3]== 'sub':
                     ls= splittamento(lis,'sub')
                     
                     ll=d[ls[j+1]]
                     d[lis[j]]=ll.append(ls[j+1])
...
def splittamento(compiti,ciao):
    l=len(compiti)
    j=0
    while j < l:
             if compiti[j][:len(ciao)] == ciao:
                 compiti[j]=compiti[j].split(ciao)
                 compiti[j].pop(0)
                 compiti[j][0]= compiti[j][0].strip()
                 compiti[j]=''.join(compiti[j][0])
             j+=1
