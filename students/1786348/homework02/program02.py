...
COMP = "comp"
PARSE_TEXT = re.compile(r"comp *(?P<ID>\d+)[sub\s]*(?P<SUB>\d*)")


def find_recursive(base, comp):
    if len(base[comp]):
        return [*find_recursive(base, base[comp][0]), base[comp][0]] if base[comp][0] != comp else [base[comp][0]]

    return []


def pianifica(fcompiti, insi, fout):
    text = open(fcompiti).read()
    instructions = dict((instr[0], [] if not instr[1] else [instr[1]]) for instr in PARSE_TEXT.findall(text))
    len_inst = len(instructions.keys())

    if sys.getrecursionlimit() < len_inst:
        sys.setrecursionlimit(len_inst)

    result = {}
    for k in insi:
        if k in instructions:
            result.update({k: find_recursive(instructions, k)})

    open(fout, "w").write(json.dumps(result))
