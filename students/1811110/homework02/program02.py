...
def diz(f):  #ricerca nel file e metti in dizionario
    lsl= [""]   #lista linee 
    a="sub"
    b="comp"
    diz = {}

    for r in f: #ogni riga del file
        lsl.append(r)
        if a in r:       #vede se c'è sub nella riga
            diz[conversione(lsl[-2])] = conversione(r)
        if b in r:      #vede se c'è comp nlla riga
            diz[conversione(r)] = []
            
    return diz


def ape(fcompiti):  #apertura 
    f=open(fcompiti)
    return f


def conversione(lsl):    #prende il numero e lo aggiunge a un sringa che ogni volta dopo averla fatta ritornare la svuota 
    strin = ''
    for i in lsl:
        if i.isdigit() != 0:
            strin += i
...
def rico(ls, y, dizio):
    if y == []:
        return ls[1:]
    ls.insert(0, dizio[y])
    return rico(ls, dizio[y], dizio)



def scrittura(fine,fout):
    import json
    json.dump(fine, open(fout,'w'))
