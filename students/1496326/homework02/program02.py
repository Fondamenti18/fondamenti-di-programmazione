...
def pianifica(fcompiti,insi,fout):
	...
	listaCompiti = {}
	with open(fcompiti) as file:
		linee = file.readlines()
		for linea in linee:
			#print(re.findall(r'\d+', linea))
			if(linea.find('comp') > -1):
				listaSub = []
				numeroComp = re.findall(r'\d+', linea)[0]
				
				listaCompiti[numeroComp] = listaSub
			
			else:
				numeroSub = re.findall(r'\d+', linea)
				
				listaCompiti[numeroComp] += numeroSub 
		
	for comp, subList in listaCompiti.items():
		if(comp in insi):
			subList = checkMultipleDependencies(subList, listaCompiti)
			listaCompiti[comp] = subList
		

	#rimuovo gli elementi non presenti in insi
	for k in list(listaCompiti.keys()):
		if(k not in insi):
			listaCompiti.pop(k, None)
			
	#salvo sul file
	with open(fout, 'w') as f:
		f.write(json.dumps(listaCompiti, ensure_ascii=False))

		
def checkMultipleDependencies(subList, listaCompiti):
	
	depList = []
	
	if(len(subList) > 0):
		sub = subList[0]
		newSubList = list(listaCompiti[sub])
		
		if(len(newSubList) > 0):
			depList = checkMultipleDependencies(newSubList, listaCompiti)
			
			for dep in newSubList:
