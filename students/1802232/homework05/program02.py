...
def ai(griglia_corrente, griglia_precedente, x, y, vx, vy, car, verso, startx, starty, laps):
    ...
    if laps == 1:
        return (-vx,-vy)
    passati.append(((x,y),(vx,vy)))
    if (x == startx and y == starty) or (x == startx and (y == starty +1 or y == starty-1)):
            return (verso,0)
    elif griglia_corrente[y+vy][x+vx] == '#' or griglia_corrente[y+vy][x+vx] == 'O':
        lib = libera(griglia_corrente,verso,x,y,passati)
        return (lib[0]+(-vx),lib[1]+(-vy))
    else:
        return (0,0)



def libera(griglia,verso,x,y,passati):
    if verso == -1:
        if passati[-2][0] == (x+1,y):
            if (griglia[y-1][x] == ' ' or griglia[y-1][x] == '|'):
                return (0,-1)
            else:
                return (0,1)
        elif passati[-2][0] == (x,y+1):
            if ((x+1,y),(1,0)) in passati:
                return (-1,0)
            elif ((x-1,y),(-1,0)) in passati:
                return (1,0)
            else:
                if (griglia[y][x+1] == ' ' or griglia[y][x+1] =='|' ) and (griglia[y][x+2] == ' ' or griglia[y][x+2] =='|' ):
                    return (1,0)
                else:
                    return (-1,0)
        elif passati[-2][0] == (x,y-1):
            if (griglia[y][x-1] == ' ' or griglia[y][x-1] == '|') and (griglia[y][x-3] == ' ' or griglia[y][x-3] == '|') and (griglia[y][x-4] == ' ' or griglia[y][x-4] == '|'):
                return (-1,0)
            else:
                return(1,0)
        else:
        ...
        if passati[-2][0] == (x+1,y):
            ...
            if ((x,y+1),(0,1)) in passati and griglia[y-1][x] == ' ':
                return(0,-1)
            elif ((x,y-1),(0,-1)) in passati and griglia[y+1][x] == ' ':
                return(0,1)
            else:
                if (griglia[y+1][x] == ' ' or griglia[y+1][x] == '|'):
                    return (0,+1)
            ...
            if (griglia[y][x-1] == ' ' or griglia[y][x-1] =='|' ) and (griglia[y][x-3] == ' ' or griglia[y][x-3] =='|' ):
                return (-1,0)
            else:
                return (1,0)
        elif passati[-2][0] == (x,y-1):
            if ((x-1,y),(-1,0)) in passati:
                ...
                if ((griglia[y][x-1] == ' ' or griglia[y][x-1] == '|') or (griglia[y+1][x-1] == ' ' or griglia[y+1][x-1] == '|')) and ((griglia[y][x-3] == ' ' or griglia[y][x-3] == '|') or (griglia[y][x-3] != 'O' and griglia[y][x-3] != '#')) or griglia[y][x+3] == '#':
                    return (-1,0)
                
                else:
                    return(1,0)
        else:
            if (griglia[y-1][x] == ' ' or griglia[y-1][x] == '|') and (griglia[y-2][x] == ' ' or griglia[y-2][x] == '|'):
                return (0,-1)
            else:
                return (0,1)