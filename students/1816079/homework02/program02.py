...
def pianifica(fcompiti,insi,fout):
    ...
    file = file.replace('omp',' ')
    file = file.replace('ub',' ')
    file = file.split()

    dictc = {}
    fdict = {}

    for t in range(0,len(file),2):
        if file[t] == 'c':
            c = file[t+1]
            dictc[c] = ''
        else:
            dictc.update({c:file[t+1]})
    ...
    for k, v in dictc.items():
        if not k in insi:
            ...
            continue
        l = [v]
        while dictc.get(l[0]) != None and dictc.get(l[0]) != '':
            l.insert(0,dictc.get(l[0]))
        fdict[k] = l
    
    with open(fout, 'w') as f:
        f.write(str(fdict).replace("'",'"'))
