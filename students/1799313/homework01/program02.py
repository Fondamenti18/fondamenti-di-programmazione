...
listanum=['','uno','due','tre','quattro','cinque','sei','sette','otto','nove','dieci','undici','dodici','tredici','quattordici','quindici','sedici','diciassette','diciotto','diciannove']
listanum2=['','dieci','venti','trenta','quaranta','cinquanta','sessanta','settanta','ottanta','novanta']
...
def conv(n):
    if n==0:
        ...
        a=n%10
        lettera=listanum2[n//10]
        if a==1 or a==8:
            lettera=lettera[:-1]
        return lettera+listanum[a]
    elif n<=199:
        b=n%100
        if 80<=b<=89:
            return "cent"+conv(b)
