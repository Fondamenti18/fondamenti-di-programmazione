...
def pianifica(fcompiti,insi,fout):
    ...
    with open (fcompiti, encoding = 'utf-8') as f:
        ...
        for elemento in lista:
            diz_elemento = cycle(elemento,testo)
            if diz_elemento == False:
                continue
            else:
                diz[elemento] = diz_elemento
        file = open(fout, 'w')
        json.dump(diz,file)

def cycle(elemento, testo):
    lista = []
    value = elemento
    while value != 0:
        value = search(value, testo)
        if value == 0:
            break
        elif value == 'Done':
            return 0
...
def search(elemento, testo):
    ...
    for lin in file:
        j+=1
        if 'comp' in lin:
            k = re.findall(r'\d+', lin)
            numero_comp = ''.join(k)
            if elemento == numero_comp:
                riga2 = file[j]
                riga2 = riga2.strip()
                if 'sub' in riga2:
                    g = re.findall(r'\d+', riga2)
                    numero_testo_sub = ''.join(g)
