...
def pianifica(fcompiti,insi,fout):
    data=dataTxt(fcompiti)
    works=cleanDict(data,insi)
    with open(fout, 'w') as file:
        json.dump(works,file)

#Create a dictionary with all the data
def dataTxt(fcompiti):
    dictionary={}
    f=open(fcompiti,'r')
    txt=f.read().replace('\n','').replace(' ','').split('comp')[1:]
    for line in txt:
        if 'sub' in line:
            dictionary[line[:line.index('s')]]=[line[line.index('b')+1:]]
        else:
            dictionary[line]=[]
    return dictionary
        
#return a dictionary of work to perform in order to complete the works on the list
def cleanDict(dictionary,insi):
    cleanDictionary={i:searchDict(dictionary,i).split(',')[1:] for i in dictionary if i in insi}
    return cleanDictionary
            
#return all the works that had to be done before the work(key) given 
def searchDict(dictionary,key):
    target=dictionary[key]
    if not target:
        return ''
    else:
        return searchDict(dictionary,target[0])+','+target[0]
