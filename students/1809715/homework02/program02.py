...
def pianifica(fcompiti, insi, fout):
    ...
    for i, el in enumerate(c):
        if 'comp' in c[i] and 'sub' in c[i + 1]:
            dict1[(c[i]).replace(' ', '').replace('comp', '')] = \
                (c[i + 1]).replace(' ', '').replace('sub', '')

        elif 'comp' in c[i] and 'sub' not in c[i + 1]:
            dict1[(c[i]).replace(' ', '').replace('comp', '')] = None
