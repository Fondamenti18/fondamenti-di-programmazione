...
def pianifica(fcompiti,insi,fout):
    #Organizzo i tasks in un dizionario
    dictTasks = tasksToDict(fcompiti)
    dictResul = dict()
    keys = dictTasks.keys()
    for idComp in insi:
        if(idComp in keys):
            dictResul[idComp] = []
            listSub = []
            if(len(dictTasks[idComp]) > 0):
                idSub = dictTasks[idComp][0]
                while idSub != -1:
                    listSub.insert(0, idSub)
                    idSub = haveSub(dictTasks, idSub)
            dictResul[idComp] = listSub
    
    with open(fout, 'w') as newJson:
        text = str(dictResul)
        text = text.replace("'", '"')
        newJson.write(text)
...
def tasksToDict(fcompiti):
    ...
    for line in text:
        ...
        if(isComp(line)):
            ...
            idList = tasks[idLastComp]
            idList.append(line[3:-1])
            tasks[idLastComp] = idList
    return tasks
...
def haveSub(dictTasks, idComp):
    if(len(dictTasks[idComp]) != 0):
        return dictTasks[idComp][0]
    else:
        return -1