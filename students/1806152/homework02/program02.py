...
def pianifica(fcompiti,insi,fout):
    ...
    f = open (fcompiti,'r') #lettura file
    testo = f.read() #variabile testo ha il file
    diz={}
    k=[]  #compiti ma senza spazi
    for elemento in insi:
        if elemento in testo:
            compiti=testo.split('comp')
            for x in compiti:
                target='s'
                if target in k:
                    indice=x.index('s')
            ...
            for key in k:
                ...
                indice=key.index('/')
                if target in key:
                    diz[key[0:indice-1]]=key[indice+1:lung]
                else :
                    diz[key[0:indice-1]]=[]
        for ele in insi:
            if ele in diz:
                diz2[ele]=subway(ele,diz)
                ris=dis2
    f.close()
    with open(fout,'w') as file_json:    
        json.dump(ris, file_json)

def subway(ele,diz):
    ls=[]
    while True :
        if diz.get(ele) != []:
            ls.insert(0,diz.get(ele))
            ele=diz.get(ele)
        else:
            break
    return(ls)
