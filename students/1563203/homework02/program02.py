...
def pianifica(fcompiti,insi,fout):
    with open(fcompiti,'rU') as f:
        comp = f.read()
        comp = comp.replace(" ","").split("\n")
        d = dict()
        fin = dict()
        for val in range(0,len(comp)): 
            if comp[val].startswith('comp')==True and comp[val+1].startswith('sub')==True:
                cod,cod2='',''
                for elm in comp[val]:
                   if elm.isnumeric()==True:
                       cod+=elm
                for elm2 in comp[val+1]:
                   if elm2.isnumeric()==True:
                       cod2+=elm2
                d[cod]=cod2
                if d[cod] in d.keys():
                    x = d[cod]
                    d[cod] = list(d[cod])+list(d[x])
                else:
                    d[cod] = list(d[cod])
            elif comp[val].startswith('comp')==True:
                cod=''
                for elm in comp[val]:
                   if elm.isnumeric()==True:
                       cod+=elm
