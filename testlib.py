...
def runtests(tests,verbose=True):
    
    results = run(tests,verbose)    
    t=sum([pair[1] for pair in results])
    metrics=['tests','cc','time']
    payload=Payload(metrics,'codice','pseudocodice')
    payload.add_exercise(1,{'tests':t,'cc':0,'time':0}) 
    #webhook=Webhook()
    #webhook.post(payload)
    print(os.environ['TRAVIS_COMMIT'])
    print(os.environ['TRAVIS_JOB_ID'])
    print('<q2a>'+payload.to_json()+'</q2a>')
